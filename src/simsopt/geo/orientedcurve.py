...
__all__ = ['OrientedCurveXYZFourier','OrientedCurveCylindricalFourier']
...
# def xyz_to rang(gamma,R0):
#     gamma_rtp = jnp.empty(gamma.shape)




# class OrientedCurveRangFourier( JaxCurve ):
#     def __init__(self,quadpoints,order,dofs= None):
#         if isinstance(quadpoints, int):
#             quadpoints = np.linspace(0,1,quadpoints,endpoint=False)
        
#         self.order = order
#         pure = lambda dofs, points: rang_centercurve_pure(dofs,points, self.order)